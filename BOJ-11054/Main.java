...
public class Main {
	...
	static int[] DP_REV = new int[MAX_SIZE];
	...
	static int Solve(int n) throws IOException {
		for (int here = 2; here <= n; here++) {
			...
			for (int i = 1; i < here; i++) {
				if(MAP[i] < MAP[here]) maxLen = Max(maxLen,DP[i]);
			}
			DP[here] = maxLen+1;
		}
		// Reverse
		for (int here = n-1; here >= 2; here--) {
			int maxLen = 0;
			for (int i = n; i > here; i--) {
				if(MAP[i] < MAP[here]) maxLen = Max(maxLen,DP_REV[i]);
			}
			DP_REV[here] = maxLen+1;
		}
		
		int ret = 0;
		for (int i = 1; i <= n; i++) {
			ret = Max(DP[i]+DP_REV[i]-1,ret); 
		}
		return ret;
	}
